...
Print the command to stdout instead of executing it.
  ...
  produces multi-line output. A command whose name collides with a shell builtin, such as `echo`, is
  printed with an `env` prefix so the real program on your `PATH` runs, matching what Raffi does when
  it launches the entry itself.
